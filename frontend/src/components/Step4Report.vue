  ...
  <div class="report-panel">
    ...
    <div class="main-split-layout">
      ...
      <div class="left-panel report-style" ref="leftPanel">
        <div v-if="reportOutline" class="report-content-wrapper">
          ...
          <div class="report-header-block">
            <div class="report-meta">
              <span class="report-tag">Consequence Assessment</span>
        ...
        <div v-if="!reportOutline" class="waiting-placeholder">
          ...
          <span class="waiting-text">Wait for Strategic Analyst...</span>
        </div>
      </div>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <button v-if="isComplete" class="next-step-btn" @click="goToInteraction">
            <span>Proceed to Interactive Analysis</span>
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <div v-if="log.details?.has_final_answer" class="final-answer-hint">
                      ...
                      <span>Section "{{ log.section_title }}" assessment complete</span>
                    </div>
                  ...
                  <template v-if="log.action === 'report_complete'">
                    <div class="complete-banner">
                      ...
                      <span>Consequence Assessment Complete</span>
                    </div>
                  </template>
                </div>
    ...
    <div class="console-logs">
      <div class="log-header">
        <span class="log-title">STRATEGIC OUTPUT</span>
...
const toggleRawResult = (timestamp, event) => {
  ...
  // Wait for DOM update, then adjust scroll position to keep button in the same position
...
const toolConfig = {
  ...
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Zap icon - represents quickness
  },
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const subQSection = text.match(/### Analyzing Sub-questions\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const relatedMatch = block.match(/Relevant Facts:\s*(\d+)/)
        return {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Parse everyone's selection reasons
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          // Save reason for previous person
    ...
    // Extract every interview record
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        // Split questions by numeric labels
      ...
      // Extract answer - partitioned into Info Plaza and Topic Community
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*Key Quotes|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Partition Info Plaza and Topic Community answers
        const twitterMatch = answerText.match(/【Info Plaza Answer】\n?([\s\S]*?)(?=【Topic Community Answer】|$)/)
        const redditMatch = answerText.match(/【Topic Community Answer】\n?([\s\S]*?)$/)
        ...
        // Platform fallback logic (compat with old formats with only one platform)
        if (!twitterMatch && redditMatch) {
          // Only Topic Community answer, copy as default only if it's not placeholder text
      ...
      if (quotesMatch) {
        ...
        // Fallback: match > "text" or > double quotes
        if (!quoteMatches) {
    ...
    const summaryMatch = text.match(/### Interview Summary and Core Insight\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/Found\s*(\d+)\s*entries/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // Extract relevant facts - full extraction, no count limit
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No currently valid memory found'),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No historical memory found'),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No involved entities found'),
  ...
  setup(props) {
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        // Q&A Conversation Thread - Conversation style
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switch buttons (only shown if real dual-platform answers exist)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading level 2 headers (## xxx), as section titles are displayed outer layer
  ...
  // Clear whitespace between list items
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Clear whitespace after list start tag
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Clear whitespace before list end tag
  ...
  // Clear <br> before/after block-level elements
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Clear cases where <p><br> is followed by block-level elements (extra empty lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clear consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clear <br> before paragraph start tag following block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: when single <ol> is separated by paragraph content, keep numbering incremental
...
const extractFinalContent = (response) => {
  ...
  // If content is long and contains markdown, try to remove thought process before returning
...
.log-msg.success { color: var(--success); opacity: 1; }
</style>
