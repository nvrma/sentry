    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Processing remaining content, manual refresh recommended later.</span>
...
// Toggle self-loop expanded/collapsed state
...
const renderGraph = () => {
  ...
  // Process edge data, calculate number of edges and indices between same node pair
  const edgePairCount = {}
  const selfLoopEdges = {} // Self-loop edges grouped by node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count edge pairs, collect self-loops
  tempEdges.forEach(e => {
  ...
  // Record current edge index per node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Calculate curvature: space out multiple edges, single edge is a straight line
    let curvature = 0
    if (totalCount > 1) {
      // Distribute curvature evenly, ensuring clear distinction
      // Curvature range increases with edge count, larger count means wider curvature
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Flip curvature if edge direction is reversed
      // Ensures all edges distribute in same reference frame, avoiding overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Adjust distance based on edge pair count
    ...
    // Add gravity towards center to group independent nodes
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Start from right of node and loop back
    ...
    // Perpendicular offset calculated by distance ratio, ensuring curve is clearly visible
    // More edges mean higher offset-to-distance ratio
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      linkGroup.selectAll('path').attr('stroke', 'var(--graph-link)').attr('stroke-width', 1.5)
      linkLabelBg.attr('fill', 'var(--graph-label-bg)')
      linkLabels.attr('fill', 'var(--graph-label-text)')
      // Highlight current selected edge
      d3.select(event.target).attr('stroke', 'var(--graph-link-active)').attr('stroke-width', 3)
  ...
  const linkLabelBg = linkGroup.selectAll('rect')
    ...
    .on('click', (event, d) => {
      ...
      link.filter(l => l === d).attr('stroke', 'var(--graph-link-active)').attr('stroke-width', 3)
  ...
  const linkLabels = linkGroup.selectAll('text')
    ...
    .text(d => d.name)
    .attr('font-size', '9px')
    .attr('fill', 'var(--graph-label-text)')
    ...
    .on('click', (event, d) => {
      ...
      // Highlight corresponding edge
      link.filter(l => l === d).attr('stroke', 'var(--graph-link-active)').attr('stroke-width', 3)
      d3.select(event.target).attr('fill', 'var(--graph-link-active)')
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if actual drag starts (moved beyond threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart simulation only on first detect of actual drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Let simulation stop gradually only after actual drag
        if (d._isDragging) {
    ...
    .on('click', (event, d) => {
      ...
      node.attr('stroke', 'var(--graph-node-stroke)').attr('stroke-width', 2.5)
      linkGroup.selectAll('path').attr('stroke', 'var(--graph-link)').attr('stroke-width', 1.5)
    ...
    .on('mouseleave', (event, d) => {
      if (!selectedItem.value || selectedItem.value.data?.uuid !== d.rawData.uuid) {
        d3.select(event.target).attr('stroke', 'var(--graph-node-stroke)').attr('stroke-width', 2.5)
      }
    })
  ...
  const nodeLabels = nodeGroup.selectAll('text')
    ...
    .attr('font-size', '11px')
    .attr('fill', 'var(--graph-node-label)')
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label position (no rotation, horizontal is clearer)
