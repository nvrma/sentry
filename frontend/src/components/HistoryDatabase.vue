<template>
  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- Card header: simulation_id and functional availability status -->
        ...
        <!-- Card description (full simulation requirements) -->
...
let pendingState = null  // Track the target state to be executed
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate number of cards in current row to ensure centering
    ...
    // Expand downwards, increase gap from title
    ...
    const x = offset * 35
    // Adjust start position, close to title but with proper gap
...
// Generate title from simulation requirements (take first 20 characters)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update target state to be executed (track latest state regardless of animation)
        ...
        // If goal state same as current, no action needed
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if target state still needs execution (may be overridden)
          ...
          // Release lock after animation and check for pending changes
          setTimeout(() => {
            isAnimating = false
            
            // Check for new target state after animation ends
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay execution slightly to avoid rapid toggling
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin, viewport bottom shrinks upwards, requires more scrolling to trigger expansion
...
onMounted(async () => {
  // Ensure data is loaded after DOM rendering
  await nextTick()
  await loadHistory()
...
// Reload data if using keep-alive when component activated
onActivated(() => {
  loadHistory()
})
...
/* Use CSS background pattern to create fixed grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Position from top-left, expands at bottom without shifting existing grid */
...
/* ===== Historical playback details modal styles ===== */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
