...
logger = get_logger('sentry.zep_entity_reader')
...
class EntityNode:
    ...
    def get_entity_type(self) -> Optional[str]:
        """Get entity type (excludes default Entity label)"""
...
class ZepEntityReader:
    ...
    2. Filter out nodes matching predefined entity types (nodes where Labels are not just Entity)
    ...
    def _call_with_retry(
        ...
        Args:
            ...
            max_retries: Maximum number of retries (default 3, max attempts is 3)
    ...
    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Total {len(edges_data)} edges retrieved")
        return edges_data
    ...
    def filter_defined_entities(
        ...
        Filtering logic:
        - If the node's Labels only contain "Entity", it means the entity does not match our predefined types, skip
        - If the node's Labels contain labels other than "Entity" and "Node", it matches a predefined type, keep
        ...
        # Get all edges (used for subsequent connection lookup)
    ...
    def get_entity_with_context(
        ...
        """
        Get a single entity and its full context (edges and connected nodes, with retry mechanism)
        ...
        try:
            ...
            # Get all nodes for connected node lookup
            all_nodes = self.get_all_nodes(graph_id)
            node_map = {n["uuid"]: n for n in all_nodes}
