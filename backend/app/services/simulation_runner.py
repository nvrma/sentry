...
logger = get_logger('sentry.simulation_runner')
...
class SimulationRunState:
    ...
    # Recent actions (for real-time display on frontend)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Rounds and time for each platform independently
    ...
    def to_detail_dict(self) -> Dict[str, Any]:
        """Includes detailed information on recent actions"""
        result = self.to_dict()
...
class SimulationRunner:
    ...
    2. Parsing runtime logs and recording each Agent's actions
    ...
    def start_simulation(
        ...
        try:
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group to ensure all related processes are terminated when server is closed
            )
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Update rounds and time for each platform independently
                                    if platform == "twitter":
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            try:
                ...
                # Force SIGKILL if not ended after timeout
    ...
    def get_all_actions(
        ...
        # If platform-specific files do not exist, try reading legacy single file format
        if not actions:
    ...
    def get_actions(
        ...
        Args:
            simulation_id: Simulation ID
            limit: Limit for returned results
            offset: Offset
    ...
    def get_timeline(
        ...
        for action in actions:
            ...
            if round_num < start_round:
                continue
            if end_round is not None and round_num > cast(int, end_round):
                continue
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Cleanup simulation run logs (used to force restart simulation)
        ...
        # Cleanup in-memory running status
        if simulation_id in cls._run_states:
    ...
    def cleanup_all_simulations(cls):
        ...
        Called when server is shut down, ensures all child processes are terminated
        """
    ...
    def register_cleanup(cls):
        ...
        # In Flask debug mode, only register cleanup in reloader sub-process (the process actually running the app)
        # WERKZEUG_RUN_MAIN=true means it's the reloader sub-process
        # If not in debug mode, this environment variable is not set, but registration is still needed
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None
        
        # In debug mode, only register in reloader sub-process; in non-debug mode, always register
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Mark as registered to prevent sub-process from trying again
            return
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            # Call original signal handler to let Flask exit properly
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None: Interview both platforms simultaneously in dual-platform simulation, returns merged results
    ...
    def interview_agents_batch(
        ...
        Returns:
            Dictionary of batch interview results
    ...
    def interview_all_agents(
        ...
        Returns:
            Dictionary of global interview results
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            raise ValueError(f"Simulation does not exist: {simulation_id}")
    ...
    def close_simulation_env(
        ...
        Sends close environment command to the simulation, making it exit gracefully from waiting-for-command mode
        ...
        try:
            ...
            # Timeout might be because the environment is closing
            return {
                "success": True,
                "message": "Environment close command sent (wait for response timed out, environment might be closing)"
            }
    ...
    def get_interview_history(
        ...
        Args:
            ...
            limit: Limit for returned results per platform
        ...
        if len(platforms) > 1 and len(results) > cast(int, limit):
