...
1. InsightForge (Deep Insight Retrieval) - Most powerful hybrid retrieval, automatically generates sub-questions and performs multi-dimensional retrieval
2. PanoramaSearch (Panorama Search) - Get the full picture, including expired content
...
logger = get_logger('sentry.zep_tools')
...
class PanoramaResult:
    """
    Panorama search results (Panorama)
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter out junk containing question numbers (Question 1-9)
                skip = False
                for d in '123456789':
                ...
                if skip:
                    continue
                # Truncate over-long content (truncate by period, not hard truncation)
...
class InterviewResult:
    ...
    Contains multiple simulated Agents' interview answers
    """
...
class ZepToolsService:
    ...
    2. panorama_search - Panorama Search (Get the full picture, including expired content)
    3. quick_search - Simple Search (Fast retrieval)
    4. interview_agents - Deep Interview (Interview simulated Agents to get multi-perspective viewpoints)
    
    【Basic Tools】
    - search_graph - Graph semantic search
    - get_all_nodes - Get all nodes in the graph
    - get_all_edges - Get all edges in the graph (including time information)
    ...
    def search_graph(
        ...
        try:
            ...
            logger.warning(f"Zep Search API failed, downgrading to local search: {str(e)}")
    ...
    def _local_search(
        ...
        """
        Local keyword matching search (as a downgrade for Zep Search API)
        ...
        # Extract search keywords (simple tokenization)
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all edges in the graph (paginated, including time information)
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        By fetching all edges of the graph and filtering those related to the specified node
    ...
    def insight_forge(
        ...
        # Also perform search for the original question
        ...
        # Step 3: Extract relevant entity UUIDs from edges, only get information for these entities (don't fetch all nodes)
        entity_uuids = set()
        ...
        # Get details for all relevant entities (not limited, full output)
        entity_insights = []
    ...
    def _generate_sub_queries(
        ...
        Decompose a complex question into multiple sub-questions that can be retrieved independently.
        """
        system_prompt = """You are a professional question analysis expert. Your task is to decompose a complex question into multiple sub-questions that can be observed independently in the simulated world.
...
Return a list of sub-questions in JSON format."""
    ...
    def panorama_search(
        ...
        Get a full-view panorama, including all relevant content and historical/expired information:
        ...
        This tool is suitable for scenarios requiring understanding the full picture of events and tracking the evolution process.
    ...
    def interview_agents(
        ...
        4. Calls /api/simulation/interview/batch interface for real interviews (simultaneous interviews on both platforms)
        ...
        # Step 2: Use LLM to select Agent for interview (returns agent_id list)
        ...
        INTERVIEW_PROMPT_PREFIX = (
            "You are participating in an interview. Please combine your profile, all past memories, and actions, "
        ...
        # Step 4: Call the real interview API (no platform specified, default simultaneous interview on both platforms)
        try:
            ...
            # Call SimulationRunner's batch interview method (platform not passed, dual-platform interview)
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                response_text = f"【Twitter Answers】\n{twitter_text}\n\n【Reddit Answers】\n{reddit_text}"

                # Extract key quotes (from answers on both platforms)
                import re
                combined_responses = f"{twitter_response} {reddit_response}"

                # Clean response text: remove markers, numbering, Markdown interference, etc.
                ...
                # Strategy 2 (Supplementary): Long text inside correctly paired quotes
                if not key_quotes:
    ...
    def _select_agents_for_interview(
        ...
        """
        Use LLM to select Agent for interview
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to select the most suitable subjects for interview from a list of simulated Agents based on interview requirements.

Selection Criteria:
1. Agent identity/profession is relevant to the interview topic
2. Agent likely holds unique or valuable perspectives
3. Choose diverse perspectives (e.g., supporters, opponents, neutrals, professionals, etc.)
...
Select up to {max_agents} most suitable Agents for the interview and explain the selection reasoning."""
    ...
    def _generate_interview_questions(
        ...
        system_prompt = """You are a professional reporter/interviewer. Generate 3-5 in-depth interview questions based on the interview requirements.

Question Requirements:
1. Open-ended questions, encouraging detailed answers
2. Different answers may be possible for different roles
...
Please generate 3-5 interview questions."""
        ...
        try:
            ...
            return [
                f"What is your viewpoint regarding {interview_requirement}?",
    ...
    def _generate_interview_summary(
        ...
        system_prompt = """You are a professional news editor. Please generate an interview summary based on the replies of multiple interviewees.

Summary Requirements:
1. Extract the main viewpoints of all parties
2. Point out consensus and differences in viewpoints
3. Highlight valuable quotes
4. Objective and neutral, no bias towards any party
...
- You can use **bold** to highlight keywords, but do not use other Markdown syntax"""
...
Please generate an interview summary."""
        ...
        try:
            ...
            # Downgrade: simple concatenation
            return f"A total of {len(interviews)} respondents were interviewed, including: " + ", ".join([i.agent_name for i in interviews])
