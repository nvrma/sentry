...
class GraphBuilderService:
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # This is the required usage for Zep SDK, and warnings from dynamic class creation can be safely ignored
        ...
        for entity_def in ontology.get("entity_types", []):
            ...
            for attr_def in entity_def.get("attributes", []):
                ...
                # Zep API requires Field's description, which is mandatory
    ...
    def add_text_batches(
        ...
        """Add text batches to graph, return list of all episode uuids"""
        episode_uuids = []
        total_chunks = len(chunks)
    ...
    def _wait_for_episodes(
        ...
        """Wait for all episodes to complete (by querying the processed status of each episode)"""
        if not episode_uuids:
            if progress_callback:
                progress_callback("No wait required (no episodes)", 1.0)
            return
        ...
        while pending_episodes:
            ...
            if progress_callback:
                progress_callback(
                    f"Zep processing... {completed_count}/{total_episodes} complete, {len(pending_episodes)} pending ({elapsed} seconds)",
