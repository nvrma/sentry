...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep does not have a built-in hybrid search interface, so edges and nodes need to be searched separately and the results combined.
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Rich information retrieved through Zep hybrid search
        """
        context_parts = []
        ...
        if entity.related_edges:
            relationships = []
            for edge in entity.related_edges:  # No limit on number
                fact = edge.get("fact", "")
        ...
        if entity.related_nodes:
            related_info = []
            for node in entity.related_nodes:  # No limit on number
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Check if it is an individual type entity"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES
    
    def _is_group_entity(self, entity_type: str) -> bool:
        """Check if it is a group/institutional type entity"""
    ...
    def _generate_profile_with_llm(
        ...
        - Individual entity: Generate specific persona settings
        ...
        # Attempt multiple generations until successful or maximum retries reached
        max_attempts = 3
        last_error = None
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    temperature=0.7 - (attempt * 0.1)  # Lower temperature for each retry
                    # Do not set max_tokens, let LLM run free
                )
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if the last quote is not followed by a comma or closing brace, the string might be truncated
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            def fix_string_newlines(match):
                s = match.group(0)
                # Replace actual newlines within string with spaces
                s = s.replace('\n', ' ').replace('\r', ' ')
            ...
            try:
                ...
                try:
                    ...
                    # Replace all multiple whitespace
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media persona generation expert. Generate detailed, realistic personas for public opinion simulation, recreating existing reality to the greatest extent. Must return valid JSON format, string values must not contain unescaped newlines. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, recreating existing reality to the greatest extent.
...
2. persona: Detailed persona description (2000 words plain text), should include:
   ...
   - Personal memory (Important part of persona, introducing the association between this individual and events, and the individual's existing actions and reactions in events)
...
- Use English (except fields specifically requested otherwise)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate detailed social media account settings for group/institutional entities, recreating existing reality to the greatest extent.
...
2. persona: Detailed account setting description (2000 words plain text), should include:
   - Institutional Basic Info (official name, nature, establishment background, main functions)
   ...
   - Posting Content characteristics (content type, frequency, active time periods)
   - Stance/Attitude (official stance on core topics, handling of controversy)
   - Special Instructions (represented group persona, operational habits)
   - Institutional Memory (Important part of persona, introducing the association between this institution and events, and the institution's existing actions and reactions in events)
3. age: Fixed as 30 (virtual age for institutional accounts)
4. gender: Fixed as "other" (institutional accounts use other to represent non-personal)
...
- persona must be a coherent text description, no newlines allowed
- Use English (except for gender field which must be "other")
- age must be integer 30, gender must be string "other"
- Institutional account speech must fit its status and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: File path for real-time output (written after each generation)
        ...
        # Helper function for real-time saving to file
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profiles as CSV format (complements OASIS official requirements)
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Use consistent format with to_reddit_format() to ensure OASIS can read it correctly.
        ...
        data = []
        for idx, profile in enumerate(profiles):
            # Use consistent format with to_reddit_format()
            item = {
                ...
                # OASIS required fields - ensure default values exist
