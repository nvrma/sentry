...
logger = get_logger('sentry.api.simulation')


# Interview prompt optimization prefix
# Adding this prefix can prevent Agent from calling tools, replying directly with text
...
def create_simulation():
    ...
    Request (JSON):
        {
            ...
            "graph_id": "sentry_xxxx",    // Optional, defaults from project if not provided
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Project has not been built yet, please call /api/graph/build first"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # Required files list (not including scripts, scripts are in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # Detailed log
        logger.debug(f"Detect simulation prep status: {simulation_id}, status={status}, config_generated={config_generated}")
        
        # If config_generated=True and files exist, consider prep completed
        # The following statuses all indicate prep work is done:
        # - ready: Prep completed, can run
        # - preparing: If config_generated=True indicates completed
        # - running: Running, indicates prep was finished long ago
        # - completed: Run completed, indicates prep was finished long ago
        # - stopped: Stopped, indicates prep was finished long ago
        # - failed: Run failed (but prep was completed)
...
def prepare_simulation():
    ...
    Features:
    - Auto detects completed prep work to avoid duplicate generation
    - If already prepared, returns existing results directly
    - Supports forced regeneration (force_regenerate=true)
    
    Steps:
    1. Check if there's already completed prep work
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Preparation task started|Prep work already completed",
                "already_prepared": true|false    // Whether prep is already completed
            }
        }
    """
    import threading
    ...
    try:
        ...
        # Check if already completed (to avoid duplicate generation)
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Prep work already completed, no duplicate generation needed",
        ...
        # This allows frontend to get expected agent total immediately after calling prepare
        try:
            logger.info(f"Synchronously getting entity count: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # Fast read entities (no need for edge info, just count)
            ...
            # Failure doesn't affect downstream process, background task will re-fetch
...
def get_prepare_status():
    ...
    2. Check if prep work already completed via simulation_id
    
    Request (JSON):
        {
            "task_id": "task_xxxx",          // Optional, task_id returned by prepare
            "simulation_id": "sim_xxxx"      // Optional, simulation ID (used to check completed prep)
        }
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "...",
                "already_prepared": true|false,  // Whether prep is already completed
                "prepare_info": {...}            // Detailed info when prep is completed
            }
        }
    """
    ...
    try:
        ...
        # If simulation_id is provided, check if prep work is already completed
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Prep work already completed",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
        ...
        if not task_id:
            if simulation_id:
                # simulation_id exists but prep not completed
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
        ...
        if not task:
            # Task does not exist, but if simulation_id is provided, check if prep is already completed
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "progress": 100,
                            "message": "Task completed (prep work already exists)",
                            "already_prepared": True,
                            "prepare_info": prepare_info
                        }
                    })
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Traverse reports directory, find matching report for simulation_id,
...
def get_simulation_history():
    ...
    Used for homepage historical projects display, returns simulation list with 
    rich information including project name, description, etc.
    ...
    try:
        ...
        # Enrich simulation data, read from Simulation file only
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use total_rounds set by user, if not exists use recommended rounds
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    - Reads file directly, not through SimulationManager
    - Suitable for real-time viewing during generation
    - Returns extra metadata (file modification time, is generating, etc.)
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "is_generating": true,  # Whether it's being generated
                "file_exists": true,
...
def get_simulation_config(simulation_id: str):
    ...
    Returns including:
        ...
        - generation_reasoning: LLM's reasoning for settings
    """
    try:
        manager = SimulationManager()
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script (General script, located in backend/scripts/)
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if the simulation is running or completed, it will be stopped and logs will be cleaned first
    ...
    About enable_graph_memory_update:
        ...
        - Uses a batch update mechanism to reduce API calls
    ...
    try:
        ...
        # Smarter status handling: if preparation is completed, allow restart
        if state.status != SimulationStatus.READY:
            # Check if preparation is completed
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # Preparation completed, check if there's a running process
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is already running, please call /stop interface first, or use force=true to force restart"
                            }), 400
                ...
                logger.info(f"Simulation {simulation_id} preparation completed, resetting status to ready (original: {state.status.value})")
...
def get_run_status(simulation_id: str):
    """
    Get real-time sim status (for frontend polling)
...
def get_run_status_detail(simulation_id: str):
    """
    Get detailed sim status (includes all actions)
    
    Used for realtime updates in frontend
    ...
    try:
        ...
        # recent_actions only shows the latest round for both platforms
...
def get_simulation_actions(simulation_id: str):
    """
    Get major agent actions in simulation
...
def interview_agent():
    ...
    Note: This feature requires simulation environment to be running (waiting for command mode after completion)
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             # Optional, specify platform (twitter/reddit)
                                               # If not specified: interview both platforms
            "timeout": 60                      # Optional, timeout (sec), default 60
        }
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or closed. Please ensure simulation has completed and entered waiting for command mode."
            }), 400
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    # Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What's your opinion on B?"  # Uses default platform if not specified
                }
            ],
            "platform": "reddit",              # Optional, default platform (overridden by per-item platform)
                                               # If not specified: interview both platforms for each agent
            "timeout": 120                     # Optional, timeout (sec), default 120
        }
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            # Required, simulation ID
            "prompt": "What's your overall opinion?", # Required, interview question (same for all agents)
            "platform": "reddit",                   # Optional, specify platform (twitter/reddit)
                                                    # If not specified: interview both platforms for each agent
            "timeout": 180                          # Optional, timeout (sec), default 180
        }
...
def close_simulation_env():
    ...
    Note: This is different from /stop interface, /stop will force terminate process,
    while this interface lets simulation close environment and exit gracefully.
    
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",  # Required, simulation ID
            "timeout": 30                  # Optional, timeout (sec), default 30
        }
