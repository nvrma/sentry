...
logger = get_logger('sentry.api.report')
...
def get_generate_status():
    ...
    try:
        ...
        # If simulation_id is provided, check if report already exists
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def get_report_sections(report_id: str):
    """
    Get generated sections list (Sectional output)
    
    Frontend can poll this interface to get generated section content without waiting for the whole report to complete
...
def get_agent_log(report_id: str):
    ...
    Get real-time every action step in report generation process, including:
    - Report start, planning start/completion
    - Start of each section, tool calls, LLM response, completion
    - Report completion or failure
    
    Query parameters:
        from_line: From which line to start reading (optional, default 0, for incremental retrieval)
