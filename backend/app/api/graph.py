...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is undergoing building, do not submit repeatedly. For forced rebuild, please add force: true",
        ...
        # Get configuration
        graph_name = data.get('graph_name', project.name or 'Sentry Graph')
        ...
        logger.info(f"Create graph building task: task_id={task_id}, project_id={project_id}")
        ...
        def build_task():
            build_logger = get_logger('sentry.build')
            try:
