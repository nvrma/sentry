...
logger = get_logger('sentry.zep_paging')
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single page request, retry with exponential backoff on failure. Only retry transient network/IO errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch graph nodes with pagination, up to max_items (default 2000). Built-in retry for each page request."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Fetch all graph edges with pagination, returning complete list. Built-in retry for each page request."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
