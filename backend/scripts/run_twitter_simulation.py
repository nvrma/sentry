...
Features:
- Do not close the environment immediately after completion; enter command waiting mode.
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter camel-ai warnings about max_tokens (we intentionally don't set it to let the model decide)."""
...
class TwitterSimulationRunner:
    ...
    # Available Twitter actions (excludes INTERVIEW, which must be triggered manually via ManualAction)
    AVAILABLE_ACTIONS = [
    ...
    def _create_model(self):
        ...
        Uniformly use configuration from project root .env file (highest priority):
        - LLM_API_KEY: API Key
        ...
        # Use config as fallback if not in .env
        if not llm_model:
...
async def main():
    ...
    # Initialize log config (use fixed filename, clean old logs)
...
def setup_signal_handlers():
    ...
    Allow program to properly clean up resources (close DB, environment, etc.).
    """
