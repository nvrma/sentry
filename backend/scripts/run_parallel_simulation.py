...
- Environment remains open after simulation completion, entering wait for command mode
...
if sys.platform == 'win32':
    ...
    # Reconfigure standard output stream to UTF-8 (resolves Chinese character issues in console)
    ...
    # Note: This needs to be set when Python starts; setting it at runtime may not take effect
...
# Load .env file in project root (contains LLM_API_KEY etc.)
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai warnings about max_tokens (we intentionally do not set max_tokens to let the model decide)"""
...
def disable_oasis_logging():
    """
    Disable detailed log output from OASIS library
    OASIS logs are too redundant (recording every agent's observation and action), we use our own action_logger
    """
...
# Twitter available actions (excludes INTERVIEW, which must be triggered manually via ManualAction)
TWITTER_ACTIONS = [
...
# Reddit available actions (excludes INTERVIEW, which must be triggered manually via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing the result, or dictionary containing error
        """
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Items that need both platforms interviewed
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows showing real entity names in actions.jsonl instead of codes like "Agent_0"
...
def fetch_new_actions_from_db(
    ...
    """
    Get new action records from the database and add complete context information
    
    Args:
        db_path: Database file path
        last_rowid: The maximum rowid value read last time (use rowid instead of created_at because creates_at formats differ between platforms)
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Streamline action_args, keep only key fields (keep full content, do not truncate)
            simplified_args = {}
            ...
            # Supplementary context information (post content, username, etc.)
...
def _enrich_action_context(
    ...
    try:
        ...
        # Retweet post: add original post content and author
        elif action_type == 'REPOST':
            new_post_id = action_args.get('new_post_id')
            if new_post_id:
                # Retweeted post's original_post_id points to original post
                cursor.execute("""
        ...
        # Create comment: Add information of the post being commented on
        ...
        # Failure to enrich context does not affect main flow
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    # Choose LLM based on parameters and configuration
...
async def run_twitter_simulation(
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed rowid in DB (uses rowid to avoid created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Also record round end even if no active agents (actions_count=0)
            if action_logger:
        ...
        # Get actually executed actions from DB and record
    ...
    # Note: Environment is not closed, kept for Interview use
...
async def main():
    ...
    if args.max_rounds:
        ...
        if args.max_rounds < config_total_rounds:
            log_manager.info(f"  - Actually executed rounds: {args.max_rounds} (Truncated)")
    log_manager.info(f"  - Agents count: {len(config.get('agent_configs', []))}")
...
def setup_signal_handlers(loop=None):
    """
    Setup signal handlers to ensure correct exit on SIGTERM/SIGINT
    
    Persistent simulation scenario: Environment remains open after simulation, waiting for interview commands.
    When a termination signal is received, we need to:
    1. Notify asyncio loop to exit wait
    2. Allow program to perform normal resource cleanup (close DB, environment, etc.)
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit (allows for resource cleanup)
            if _shutdown_event:
                _shutdown_event.set()
